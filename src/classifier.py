...
class Classifier(object):
    def __init__(
        self,
        args: CfgNode,
        base_weight: torch.FloatTensor,
        base_bias: torch.FloatTensor,
        n_tasks: int
    ):
        self.num_base_classes_and_bg: int = base_weight.size(-1)
        self.num_novel_classes: int = args.num_classes_val
        self.num_classes: int = self.num_base_classes_and_bg + self.num_novel_classes
        self.n_tasks: int = n_tasks

        # Snapshot of the model right after training, frozen
        self.snapshot_weight: torch.FloatTensor = base_weight.squeeze(
            0).squeeze(0).clone()
        self.snapshot_bias: torch.FloatTensor = base_bias.clone()

        # [n_tasks, c, num_base_classes_and_bg]
        self.base_weight: torch.FloatTensor = base_weight.squeeze(0).repeat(
            self.n_tasks, 1, 1)

        # [n_tasks, num_base_classes_and_bg]
        self.base_bias: torch.FloatTensor = base_bias.unsqueeze(0).repeat(
            self.n_tasks, 1)

        self.novel_weight, self.novel_bias = None, None
        self.pi, self.true_pi = None, None

        self.fine_tune_base_classifier: bool = args.fine_tune_base_classifier
        self.lr: float = args.cls_lr
        self.adapt_iter: int = args.adapt_iter
        self.weights: list[int] = args.weights
        self.pi_estimation_strategy: str = args.pi_estimation_strategy
        self.pi_update_at: list[int] = args.pi_update_at

    @staticmethod
    def _valid_mean(
        t: torch.FloatTensor,
        valid_pixels: torch.FloatTensor,
        dim: int
    ) -> torch.FloatTensor:
    ...
    def init_prototypes(
        ...
        ds_gt_s: torch.FloatTensor = F.interpolate(
            gt_s.float(), size=features_s.shape[-2:], mode='nearest')
        # [n_novel_classes, shot, 1, h, w]
        ds_gt_s = ds_gt_s.long().unsqueeze(2)

        # Computing prototypes
        self.novel_weight = torch.zeros(
            (features_s.size(2), self.num_novel_classes), device=features_s.device)
        for cls in range(self.num_base_classes_and_bg, self.num_classes):
            novel_mask = (ds_gt_s == cls)
            novel_prototype = self._valid_mean(
                features_s, novel_mask, (0, 1, 3, 4))  # [c,]
            self.novel_weight[:, cls -
                              self.num_base_classes_and_bg] = novel_prototype
        ...
        self.novel_weight = self.novel_weight.unsqueeze(
    ...
    def get_base_snapshot_probas(self, features: torch.Tensor) -> torch.Tensor:
        ...
        """
        logits = torch.einsum('bochw,cC->boChw', features,
                              self.snapshot_weight) + self.snapshot_bias.view(1, 1, -1, 1, 1)
        return torch.softmax(logits, dim=2)
    ...
    def image_level_supervision_pi(
        self,
        features_q: torch.Tensor,
        unsqueezed_valid_pixels_q: torch.Tensor
    ) -> torch.Tensor:
    ...
    def compute_pi(
        ...
        """
        valid_pixels_q = F.interpolate(valid_pixels_q.float(
        ), size=features_q.size()[-2:], mode='nearest').long()
        valid_pixels_q = valid_pixels_q.unsqueeze(2)

        if gt_q is not None:
            ds_gt_q = F.interpolate(
                gt_q.float(), size=features_q.size()[-2:], mode='nearest').long()
            # [batch_size_val, shot, num_classes, h, w]
            one_hot_gt_q = to_one_hot(ds_gt_q, self.num_classes)
            self.true_pi = self._valid_mean(
                one_hot_gt_q, valid_pixels_q, (1, 3, 4))
        ...
        if self.pi_estimation_strategy == 'upperbound':
            ...
            self.pi = self.image_level_supervision_pi(
                features_q, valid_pixels_q)
        elif self.pi_estimation_strategy == 'uniform':
            pi = 1 / self.num_classes
            # [batch_size_val, num_classes]
            self.pi = torch.full_like(self.true_pi, pi)
        else:
    ...
    def distillation_loss(
        ...
        adjusted_curr_p[:, :, 0, ...] += curr_p[:, :,
                                                self.num_base_classes_and_bg:, ...].sum(dim=2)
        kl = (adjusted_curr_p * torch.log(1e-10 +
              adjusted_curr_p / (1e-10 + snapshot_p))).sum(dim=2)
    ...
    def get_ce(
        ...
        probas = probas.clone()
        probas[:, :, 0, ...] += probas[:, :,
                                       1:self.num_base_classes_and_bg, ...].sum(dim=2)
    ...
    def optimize(
        ...
        ds_gt_s = F.interpolate(
            gt_s.float(), size=features_s.size()[-2:], mode='nearest').long()
        # [1, num_novel_classes * shot, num_classes, h, w]
        one_hot_gt_s = to_one_hot(ds_gt_s, self.num_classes)
        ...
        for iteration in range(self.adapt_iter):
            # logits
            logits_s, logits_q = self.get_logits(
                features_s), self.get_logits(features_q)

            # probability
            proba_s, proba_q = self.get_probas(
                logits_s), self.get_probas(logits_q)

            snapshot_proba_q = self.get_base_snapshot_probas(features_q)

            # distillation
            distillation = self.distillation_loss(
                proba_q, snapshot_proba_q, valid_pixels_q, reduction='none')

            # transductive inference
            d_kl, entropy, marginal = self.get_entropies(
                valid_pixels_q, proba_q, reduction='none')
